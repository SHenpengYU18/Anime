...
async def video_links(watch_episode_url,browser):
    ...
    try:
        ...
        print(f'打开视频页面出错: {e}')
    return video_link_container[0] if video_link_container else []
...
async def anime_main():
    async with async_playwright() as p:
        ...
        for weekday in range(1,8):
            ...
            await page.wait_for_load_state('networkidle')
            time.sleep(0.5)
        await page.click('a.week-key7')
        await page.wait_for_load_state('networkidle')

        with open('task_res','a',encoding='utf-8') as f:
            for anime_info in ANIME_QUEUE:
                ret=await anime_video(anime_info,browser)
